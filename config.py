# -*- coding: utf-8 -*-
"""config.py -- Phantom Edge Bot v6.2 — ZigZag Institutional Elite (Pine-matched)."""
import os
...
class Config:
    ...
    trade_usdt:  float = 6.0     # minimum per trade
    leverage:    int   = 10      # 10x as requested

    # ── Strategy (matches Pine script) ────────────────────────
    pivot_len:   int   = 5       # ta.pivothigh(high, 5, 5)
    hma_len:     int   = 50      # compat only
    ft_period:   int   = 25      # compat only
    atr_period:  int   = 14
    atr_mult:    float = 2.0     # SL buffer for exchange order
    rr:          float = 2.0     # Pine: tp_mult=2.0

    # ── Filters ───────────────────────────────────────────────
    min_atr_pct:  float = 0.10   # skip flat markets
    min_vol_mult: float = 1.5    # Pine: vol_mult=1.5 (institucional_vol)
    min_score:    int   = 3      # base score from 3 conditions; 4 with 15m confirm
    ...
    def __post_init__(self) -> None:
        ...
        self.trade_usdt         = max(6.0, float(e("TRADE_USDT",       str(self.trade_usdt))))
